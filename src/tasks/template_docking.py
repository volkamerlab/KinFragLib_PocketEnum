...
def template_docking_task(config: Config, subpocket: str, recombination: Recombination, poses: list):
    ...
    for i, pose in enumerate(poses):
        ...
        res_docking = _template_docking(config.path_temp /('cand_thread_' + str(thread_id) + '_' + subpocket + '_fragment.sdf'), config.path_temp / ('temp_thread_' + str(thread_id) + '_' + subpocket + '_fragment.sdf'), 
        ...
        if len(res_docking) and config.use_hyde:
            res_hyde = hyde_scoring(config.path_temp / ('dock_thread_' + str(thread_id) + '_' + 'fragments.sdf'), config.path_structure_config / (subpocket + '.hydescorer'), 
            ...
            remove_files(config.path_temp / ('dock_thread_' + str(thread_id) + '_' + 'fragments.sdf'), config.path_temp / ('temp_thread_' + str(thread_id) + '_' + subpocket + '_fragment.sdf'),
    ...
    remove_files(config.path_temp /('cand_thread_' + str(thread_id) + '_' + subpocket + '_fragment.sdf'))
...
def _template_docking(path_fragment: Path, path_template: Path, path_config: Path, path_output: Path, path_flexx: Path, ligand: Ligand, print_output=False) -> list:
    """
