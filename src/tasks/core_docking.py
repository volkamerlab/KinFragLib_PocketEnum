...
def core_docking_task(config: Config, core_fragment):
    ...
    res_docking = _core_docking(config.path_temp/ ('cand_thread_' + str(thread_id) + '_core_fragment.sdf'), config.path_structure_config / (config.core_subpocket + '.flexx'), 
                                             config.path_temp / ('dock_thread_' + str(thread_id) + '_core_fragment.sdf'), config.path_flexx, core_fragment)
    ...
    if config.use_hyde and len(res_docking):
        ...
        res_hyde = hyde_scoring(config.path_temp / ('dock_thread_' + str(thread_id) + '_core_fragment.sdf'), config.path_structure_config / (config.core_subpocket + '.hydescorer'), 
                                              config.path_temp / ('hyde_thread_' + str(thread_id) + '_core_fragment.sdf'), config.path_hyde,
                                              core_fragment)

        remove_files(config.path_temp / ('dock_thread_' + str(thread_id) + '_core_fragment.sdf'), config.path_temp/ ('cand_thread_' + str(thread_id) + '_core_fragment.sdf'), 
...
def _core_docking(path_fragment: Path, path_config: Path, path_output: Path, path_flexx: Path, ligand: Ligand, print_output=False) -> list:
    """
    runs FlexX docking
