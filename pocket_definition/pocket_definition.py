...
if __name__ == '__main__':
    ...
    AMINO_ACIDS = {'CYS': 'C', 'ASP': 'D', 'SER': 'S', 'GLN': 'Q', 'LYS': 'K',
        ...
        'ALA': 'A', 'VAL':'V', 'GLU': 'E', 'TYR': 'Y', 'MET': 'M', 'SEP': '?', 'TPO': '?'}
    ...
    parser.add_argument('-p', '--pdb_structure', help='PDB ID of the structure of interest e.g. 3AMB', required=True)
    ...
    with open("pocket_sequences.fasta", "wt") as fasta_file:
        for i, structure in enumerate(structures):
            fasta_file.write(">" +  structure.pdb + str(structure.structure_ID) + str(i) + "\n" + structure.pocket + "\n")

    # perform MSA with clustalo
    output_text = subprocess.run(
        ["clustalo",
        "-i", "pocket_sequences.fasta",
        "-o", "alignment.clustal",
        "--outfmt=clu", "--dealign",
        "--force"]
        ,capture_output=True
    ...
    seq1 = SeqRecord(Seq(consensus_sequence),
                    id="seq1")
    seq2 = SeqRecord(Seq(sequence),
                    id="seq2")
    ...
    hsp = next(hsp for aligment in blast_result_record.alignments for hsp in aligment.hsps if hsp.query_start == 1 and hsp.query_end == 85)
